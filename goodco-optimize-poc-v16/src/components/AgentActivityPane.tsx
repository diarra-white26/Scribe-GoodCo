...
export function AgentActivityPane({ pipelineId, stage, inheritedChips, onChipsReady }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4 px-6 py-5">
      ...
      {/* Escalated stages (Path B1/B2) fold Intent/Priority/Customer/confidence
          directly into the analyst dashboard's own header instead of a
          separate Output box above it — one artifact, not two competing
          summaries of the same classification. Path A keeps the Output box
          since it's the only summary before the auto-resolved chat script. */}
      {!stage.resolutionViews?.escalated && (
