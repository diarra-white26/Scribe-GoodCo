...
export const pipeline: Pipeline = {
  ...
  stages: [
    ...
    },
    {
      ...
      businessPurpose: "This is the exception path, for cases the agent shouldn't resolve on its own: security-sensitive activity, low-confidence classification, anything outside the known Tier-1 intents. Analysts still own these.",
      sources: [
      ...
      scripts: [
        {
          ...
          agentSteps: [
            {
              title: 'Looks up the account',
              detail: "This is the exception path, for cases the agent shouldn't resolve on its own: security-sensitive activity, low-confidence classification, anything outside the known Tier-1 intents. It starts the same way as Path A, with the same classification and account lookup, including the same urgency call: this is still an access-blocking issue, so urgency is high either way.",
            },
            {
              title: 'Flags the exception',
              detail: "Recognizes the lookup is atypical: 4 failed SSO attempts from a new device in the last hour. Urgency stays high, access is still blocked, but confidence drops to 0.52, below the 0.85 auto-resolve threshold, because this reads as a possible security event, not a routine reset.",
            },
            {
      ...
      contextOut: [
        ...
        {
          id: 'chip-path-b-security-flag',
          label: 'Security flag logged for Finance & Security review: repeated failed sign-in on an enterprise account',
          detail: 'The same flag reason handed to the Tier-2 analyst also lands in the security log Finance and IT Security already review for anomalous account activity, no separate report needed.',
          fromStageId: 'path-b-escalated',
          emoji: 'shield',
        },
      ],
