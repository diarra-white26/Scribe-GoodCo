  ...
  return (
    <div className="space-y-3">
      <div className="rounded-md border border-line-soft bg-stage-2 p-3">
        <p className="text-[13px] text-ink-4 mono">{artifact.ticket.requester}</p>
        <p className="text-[15.5px] font-medium text-ink mt-1">{artifact.ticket.subject}</p>
        <p className="text-[14.5px] text-ink-3 leading-snug mt-1 text-pretty">{artifact.ticket.body}</p>
      </div>
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          <p className="mono text-[12px] tracking-[0.18em] uppercase text-ink-4 mb-1.5">Before</p>
        ...
        >
          <div className="flex items-center justify-between mb-1.5">
            <p className="mono text-[12px] tracking-[0.18em] uppercase text-os">After</p>
...
function FieldRow({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="flex items-center justify-between py-0.5">
      ...
      <span className={`text-[14px] font-medium ${accent ? 'text-ink' : 'text-ink-3'}`}>{value}</span>
    </div>
  );
}
