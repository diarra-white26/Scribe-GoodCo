...
export function ContextRail({
  ...
  return (
    <div className="w-full h-full bg-stage-2 flex flex-col min-h-0">
      <div className="px-4 pt-4 pb-2 border-b border-line-soft">
        <div className="flex items-baseline justify-between">
          <p className="mono text-[10px] tracking-[0.22em] uppercase text-os">Memory</p>
          <p className="mono text-[10px] text-ink-4">{workingCount} {workingCount === 1 ? 'entry' : 'entries'}</p>
        </div>
        <p className="text-ink-3 text-[12px] mt-1 leading-snug">What this stage is drawing on, reusing, and producing.</p>
      </div>

      <div className="relative flex-1 min-h-0 overflow-y-auto px-4 py-3" ref={memoryRef}>
        <MemoryGraph
        ...
        <section className="relative">
          ...
          {openSources && (
            <>
              {sources.length === 0 ? (
                <p className="text-ink-4 text-[12px] italic text-center leading-relaxed py-3">This stage runs on memory alone.</p>
              ) : (
                  ...
                  {sources.map((src) => (
                    ...
                    >
                      ...
                      <span className="flex-1 min-w-0 flex flex-col">
                        <span className="text-[12.5px] text-ink leading-snug truncate">{src.label}</span>
                        {src.detail && (
                          <span className="text-[10.5px] text-ink-4 leading-snug truncate">{src.detail}</span>
                        )}
                      </span>
                    </li>
                  ))}
        ...
        <section className="mt-4 pt-3 border-t border-line-soft">
          ...
          {openLibrary && (
            <>
              {workingCount === 0 ? (
                <p className="text-ink-4 text-[12px] italic mt-3 text-center leading-relaxed">Outputs land here as the stage finishes.</p>
              ) : (
          ...
          <section className="mt-4 pt-3 border-t border-line-soft">
            ...
            >
              <span className="flex items-center gap-1.5">
                ...
                <span className="mono text-[10px] tracking-[0.22em] uppercase text-ink-3">View earlier memory</span>
              </span>
              <span className="mono text-[10px] text-ink-4">{historyChips.length}</span>
            </button>
            {openHistory && (
                ...
                {historyChips.map((c, i) => {
                  ...
                  return (
                    <li
                      ...
                      className="relative flex items-start gap-2 pl-6 pr-2 py-1.5 rounded-md border border-line-soft bg-panel"
                    >
                      <span className="absolute left-2 top-1.5 mono text-[10px] text-ink-4 select-none">{String(i + 1).padStart(2, '0')}</span>
                      <span className="flex-1 min-w-0 flex flex-col">
                        <span className="text-[12.5px] text-ink leading-snug truncate flex items-center gap-1.5" title={c.label}>
                          {isIcon ? (
                            <Icon name={c.emoji as IconName} size={12} className="text-os flex-shrink-0" />
                          ) : c.emoji ? (
                            <span aria-hidden className="text-[12px] flex-shrink-0">{c.emoji}</span>
                          ) : null}
                          <span className="truncate">{c.label}</span>
                        </span>
                        <span className="text-[10.5px] text-ink-4 mono leading-snug">
                          Stage {stageOrdinal[c.fromStageId] ?? '\u00b7'}
                        </span>
                      </span>
                      {refined.has(c.id) && (
                        <span className="mono text-[9px] tracking-[0.18em] uppercase text-os bg-os/8 border border-os/30 rounded-full px-1.5 py-[1px] flex-shrink-0 self-center">
                          Refined
                        </span>
                      )}
                    </li>
      ...
      <div className="px-4 py-2.5 border-t border-line-soft flex items-center justify-between">
        <span className="mono text-[10px] tracking-[0.22em] uppercase text-ink-4">Memory size</span>
        <span className="mono text-[12px] text-os">{sources.length + workingCount}</span>
      </div>
    </div>
  );
}
...
function SectionHeader({ label, count, unit, open, onToggle }: SectionHeaderProps) {
  return (
    ...
    >
      <span className="flex items-center gap-1.5">
        <Chevron open={open} />
        <span className="mono text-[10px] tracking-[0.22em] uppercase text-ink-3">{label}</span>
      </span>
      <span className="mono text-[10px] text-ink-4">{count} {unit}</span>
    </button>
  );
}
...
function ChipGroup({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="mono text-[9.5px] tracking-[0.22em] uppercase text-ink-4 mb-1">{label}</p>
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <p className="text-ink text-[12.5px] font-medium leading-snug flex-1 min-w-0">{chip.label}</p>
            {refined ? (
              <span className="mono text-[9px] tracking-[0.18em] uppercase text-os bg-os/8 border border-os/30 rounded-full px-1.5 py-[1px] flex-shrink-0">
                Refined
              </span>
            ) : (
              <span className="mono text-[9px] tracking-[0.18em] uppercase text-ink-4 border border-line rounded-full px-1.5 py-[1px] flex-shrink-0">
                {tag}
              </span>
            )}
          </div>
          {rationale && tag === 'Reused' && (
            <p className="text-os/85 text-[11px] leading-snug mt-1 text-pretty">Needed for: {rationale}</p>
          )}
          <motion.p
            key={detailKey}
            ...
            className={`text-[11.5px] leading-snug mt-1 text-pretty ${refined ? 'text-ink-2' : 'text-ink-3'}`}
          >
            {refined && (
              <span className="mono text-[9.5px] tracking-[0.18em] uppercase text-os mr-1.5 align-baseline">
                Now ·
              </span>
            )}
            {displayDetail}
