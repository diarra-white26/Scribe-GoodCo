...
function SourceRow({ items }: { items: NonNullable<Stage['sources']> }) {
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <span className="mono text-[10px] tracking-[0.2em] uppercase text-ink-4 flex-shrink-0">Pulling from</span>
      <div className="flex flex-wrap gap-1.5">
        {items.map((item) => (
          <span
            key={item.id}
            title={item.detail}
            className="text-[12px] px-2 py-0.5 rounded-full border border-line bg-panel text-ink-2 max-w-[240px] truncate"
          >
            {item.label}
          </span>
        ))}
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-start gap-2 flex-wrap">
        <span className="mono text-[10px] tracking-[0.2em] uppercase text-os flex-shrink-0 mt-1">Reusing</span>
        <div className="flex flex-col gap-1.5 flex-1 min-w-0">
          {chips.map((c, i) => (
            ...
            >
              <span className="text-[12px] px-2 py-0.5 rounded-full border border-os/35 bg-os/8 text-os max-w-[260px] truncate flex-shrink-0">
                {c.label}
              </span>
              {rationale[c.id] && (
                <span className="text-[11.5px] text-ink-3 leading-snug pt-0.5">
