 ...
 * walkthrough, now with the escalation lane split in two:
...
export const pipeline: Pipeline = {
  ...
  wrapStats: [
    ...
    { label: 'Confidence', before: '—', after: 'High on Path A, human-confirmed on both escalation paths' },
  ],
  stages: [
    {
    ...
    },
    {
      ...
      index: 1,
      label: 'Path B1 · Escalated from a Tier-1 attempt',
      shortLabel: 'Path B1',
      ...
      sources: [
        ...
        { id: 'src-escalation-queue-b1', label: 'Tier-2 escalation queue', detail: 'Where flagged tickets land with full context attached' },
      ],
      scripts: [
        {
          agentId: 'virtual-agent',
      ...
      contextOut: [
        {
          ...
          label: 'Escalated to Tier-2: unusual sign-in activity, Tier-1 · high, 0.52 confidence',
      ...
      resolutionViews: {
        escalated: {
          ...
          analystDecision:
            'Priya called the customer at the phone number on file, confirmed their identity, and cleared the flagged device before re-provisioning SSO access. Nothing changed on the account until a person verified it should.',
          finalEmail:
            "Hi, this is Priya from GoodCo support. Thanks for confirming your identity by phone. I've cleared the flagged device and re-provisioned your SSO access, you should be able to log in normally now. Let me know if anything else comes up.",
          ticketStatus:
            'Closed — identity verified by phone, device cleared, SSO access restored. Flagged device marked resolved in the security log.',
        },
      },
    ...
    },
    {
      ...
      sources: [
        { id: 'src-access-control-api', label: 'Building access control API', detail: 'Reader status, connectivity, and firmware/event logs per site' },
        { id: 'src-escalation-queue-b2', label: 'Tier-2 escalation queue', detail: 'Where flagged tickets land with full context attached' },
      ],
      scripts: [
        {
          agentId: 'virtual-agent',
          agentName: 'Virtual agent',
          agentRole: 'Classifies, recognizes this is outside Tier-1 scope, escalates with context',
          agentSteps: [
            {
              title: 'Classifies the ticket',
              detail: "Runs the same classification step as Path A: category, product area, intent, and urgency. This one doesn't match any of the known Tier-1 intents, password reset, lockout, SSO, 2FA, seat transfer, it's a hardware and access-control fault. Urgency uses the same heuristic as Path A: this blocks physical building access for a large group, so it's high regardless of intent.",
            },
            {
              title: 'Runs an initial diagnostic',
              detail: 'Pulls reader connectivity, status, and recent firmware/event logs through the building access control API, the same check an analyst would run by hand today.',
            },
            {
              title: 'Packages the context',
              detail: 'Bundles the full ticket thread, site and device details, and diagnostic results into one handoff, instead of a summary the analyst has to reconstruct from scratch.',
            },
            {
              title: 'Escalates to Tier-2',
              detail: "Routes to a human analyst with everything attached. This was never a Tier-1 candidate, so there's no failed auto-resolve attempt first, just a fast, well-documented handoff.",
            },
          ],
        },
      ],
      contextOut: [
        {
          ...
          label: 'Escalated to Tier-2: badge reader connectivity fault, Tier-2 · high, outside known Tier-1 intents',
          detail: 'Full ticket thread, site, and diagnostic context handed to the analyst, nothing repeated to the customer.',
        ...
        },
        {
          ...
          detail: 'The same diagnostic and resolution details handed to the Tier-2 analyst also feed the physical security incident log Security already reviews for building access anomalies, no separate report needed.',
          fromStageId: 'path-b2-tier2-from-start',
          emoji: 'shield',
        },
      ],
      artifact: {
        ...
        accountLookup: {
          ...
          detail: [
            { label: 'Affected readers', value: '6, offline since 8:54am' },
            { label: 'Likely cause', value: 'Overnight firmware push' },
          ],
        },
        confidence: 0.12,
        agentAction: 'escalated-tier-2',
      ...
      resolutionViews: {
        escalated: {
          label: 'Tier-2 · escalated to a real analyst',
          trigger: "Multiple badge readers on one floor lost connectivity overnight and are denying valid badge-ins, a hardware and access-control fault outside any known Tier-1 intent, not a routine account request.",
          confidence: 0.12,
          priorChat: [
            {
              from: 'virtual-agent',
          ...
          analystContext: [
            ...
            'Diagnostic pulled automatically: all 6 readers lost connection to the access control panel at 8:54am, after an overnight firmware push',
          ],
          recommendedNextStep:
            'Run a remote connectivity reset on the affected readers first; if that clears it, confirm with the customer before closing. If not, dispatch a technician given the number of employees affected.',
          draftEmail:
            "Hi, this is Priya from GoodCo support. I can see the badge readers on your 4th floor engineering wing lost connection overnight after a firmware update on our end. I'm running a remote reset now and will confirm as soon as access is restored.",
          analystDecision:
            'Priya ran the remote connectivity reset, confirmed all 6 readers reconnected within a few minutes, and called the site contact to verify badges were working again before closing, no technician dispatch needed.',
          finalEmail:
            "Hi, this is Priya from GoodCo support. Good news, the badge readers on your 4th floor engineering wing are back online. The issue was a connectivity drop after an overnight firmware update on our end, I've pushed a fix remotely and confirmed all 6 readers are responding normally. I've also flagged the firmware push to our engineering team so this doesn't happen again. Let me know if anyone's still having trouble getting in.",
          ticketStatus:
            'Closed — remote fix confirmed on-site, no technician dispatch needed. Root cause flagged to Engineering to prevent recurrence.',
        },
      },
