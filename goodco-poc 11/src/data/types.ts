...
export interface AgentStep {
  title: string;
  /** Shown on the back of the flip-card when a person clicks the step. */
  detail: string;
}
...
export interface ResolutionViews {
  ...
  escalated?: {
    ...
    draftEmail: string;
    /** What the analyst actually decided/did after following the recommended
     *  next step, shown alongside the draft so the dashboard tells the whole
     *  story, not just the handoff. */
    analystDecision: string;
    /** The email as it actually went out, after the analyst's edits. Shown
     *  side by side with draftEmail so an edit is visible, not assumed. */
    finalEmail: string;
