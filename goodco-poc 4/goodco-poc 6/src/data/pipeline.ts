...
export const pipeline: Pipeline = {
  ...
  title: 'A new ticket comes in',
  subtitle: 'GoodCo · Optimize recommendation, Tier-1 support automation',
      ...
      scripts: [
        {
          ...
          agentActionBullets: [
            'Reads the incoming ticket and classifies category, product area, and intent (trained on 18 months of resolved tickets)',
            "Looks up the customer's account status, last login, and SSO provider through the admin API",
            'Resolves the request directly in chat when confidence is 0.85 or higher',
            'Closes the ticket with no analyst involved, this is the default path for Tier-1',
          ],
        },
      ],
      evalLines: [
        ...
        'Forrester estimates each manual password reset costs about $70 in help-desk labor; this path targets that cost directly',
        'Tier-1 is automatic by default in this design, not human-reviewed by default. The safety valve is Path B, not a person checking every case.',
      ],
      contextOut: [
        {
      ...
      scripts: [
        {
          ...
          agentActionBullets: [
            "Looks up the customer's account status the same way as Path A",
            'Recognizes the account lookup is atypical: 4 failed SSO attempts from a new device in the last hour',
            'Stops short of resolving, confidence drops to 0.52, below the 0.85 auto-resolve threshold',
            'Escalates to a Tier-2 analyst, carrying the full chat history and lookup context forward so nothing has to be repeated',
          ],
        },
      ],
      evalLines: [
        'The escalation trigger is intentionally conservative: unusual account activity or low classification confidence routes to a person rather than guessing',
        'The analyst receives the full prior chat and account context in one screen, this is a warm handoff, not a cold one',
        'This is the safety valve for the 5 to 20% of Tier-1-shaped tickets that well-configured automation typically can\'t resolve on its own',
      ],
      contextOut: [
        {
