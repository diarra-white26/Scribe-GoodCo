  ...
  return (
    <div className="rounded-lg border border-line-soft bg-panel overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row">
        ...
        {/* Main: tabbed conversation/customer/activity panel left, reply
            composer docked right — replying is the one action relevant no
            matter which tab you're reading, so it stays put. */}
        <div className="flex-1 min-w-0 grid grid-cols-1 lg:grid-cols-[1.3fr_1fr] gap-4 p-4">
          <TicketTabs stage={stage} escalated={escalated} meta={meta} customerLine={customerLine} />
...
/** The three clickable tabs: Conversation (the ticket thread, default),
 *  Customer (identification info, the kind of panel a Zendesk agent view
 *  keeps down the side), and Activity (ticket context, the classification
 *  reasoning behind the route, and the recommended next step). */
function TicketTabs({
  stage,
  escalated,
  meta,
  ...
  stage: Stage;
  escalated: NonNullable<ResolutionViews['escalated']>;
  meta: { ticketId: string; age: string };
  customerLine: string;
}) {
  const [tab, setTab] = useState<'Conversation' | 'Customer' | 'Activity'>('Conversation');
  const tabs: Array<'Conversation' | 'Customer' | 'Activity'> = ['Conversation', 'Customer', 'Activity'];

  return (
    <div className="space-y-2 min-w-0">
      <TabStrip active={tab} tabs={tabs} onSelect={setTab} />
      ...
      {tab === 'Customer' && (
        <div className="rounded-lg border border-line-soft bg-stage-2 p-3.5 space-y-3">
          <div>
            <p className="mono text-[10.5px] tracking-[0.16em] uppercase text-ink-4 mb-1">Organization</p>
            <p className="text-[14px] text-ink font-medium">{customerLine}</p>
          </div>
          <div className="grid grid-cols-2 gap-x-4 gap-y-2.5">
            {stage.artifact.accountLookup.detail.map((d) => (
              <div key={d.label} className="min-w-0">
                ...
                <p className="text-[12.5px] text-ink-2 truncate">{d.value}</p>
              </div>
            ))}
            <div className="min-w-0">
              <p className="text-[10px] uppercase tracking-[0.1em] text-ink-4 truncate">Channel</p>
              <p className="text-[12.5px] text-ink-2 truncate">Chat widget</p>
            </div>
            <div className="min-w-0">
              <p className="text-[10px] uppercase tracking-[0.1em] text-ink-4 truncate">Ticket ID</p>
              <p className="text-[12.5px] text-ink-2 truncate">{meta.ticketId}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {tab === 'Activity' && (
        <div className="rounded-lg border border-line-soft bg-stage-2 p-3.5 space-y-3">
          <div>
            <p className="mono text-[11px] tracking-[0.16em] uppercase text-ink-4 mb-1">Classification justification</p>
            <p className="text-[13px] text-ink-2 leading-snug text-pretty">{escalated.trigger}</p>
            <ThresholdNote />
          </div>
...
/** Clickable tab strip, styled like a real Zendesk/Front-style ticket
 *  workspace — switches between the conversation, the customer record, and
 *  the classification/activity trail. */
function TabStrip<T extends string>({ active, tabs, onSelect }: { active: T; tabs: T[]; onSelect: (t: T) => void }) {
  return (
    <div className="flex items-center gap-1 border-b border-line-soft">
      {tabs.map((t) => (
        <button
          ...
          type="button"
          onClick={() => onSelect(t)}
          className={`text-[12px] px-2.5 py-1.5 -mb-px border-b-2 transition-colors ${
            t === active ? 'text-ink border-os font-medium' : 'text-ink-4 border-transparent hover:text-ink-2'
          }`}
        >
          {t}
        </button>
