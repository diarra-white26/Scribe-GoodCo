 ...
 * what the agent did (why it routed here), what it's handing off (ticket
 * context), what it recommends doing next, and a drafted email sitting ready
 * for the analyst to review and send, mirroring the source recommendation's
 * own design — "agent reviews and clicks Send, no auto-send in v1." Not a
 * cold handoff.
 */
    ...
    <div className="rounded-lg border border-line-soft bg-panel overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1.3fr_1fr] gap-4 p-4">
        <div className="space-y-2">
          ...
          <ChatWidget headerName="Support ticket" headerSubtitle="Routed · awaiting Priya" statusColor="bg-amber">
...
/** A drafted email, styled with a light "To" line so it reads as an email
 *  rather than another chat bubble — the outbound channel for Path B is
 *  email, distinct from the ticket thread above it. */
function EmailBox({ eyebrow, body, footer }: { eyebrow: string; body: string; footer?: string }) {
  return (
    <div className="rounded-md border border-line-soft bg-stage-2 p-2.5">
      <p className="mono text-[11px] tracking-[0.16em] uppercase text-ink-4 mb-1.5">{eyebrow}</p>
